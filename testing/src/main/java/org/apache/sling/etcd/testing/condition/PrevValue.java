...
public class PrevValue implements Condition {

    private final String value;

    public PrevValue(@Nonnull String value) {
        this.value = value;
    }
    ...
    public void check(@Nonnull String key, @Nullable Node node, long index)
        ...
        if (node.isFolder()) {
            throw new EtcdException(ErrorCodes.NOT_FILE, key, index);
        }

        Key k = (Key) node;

        if (! value.equals(k.value())) {
            String cause = String.format("[%s != %s]", value, k.value());
